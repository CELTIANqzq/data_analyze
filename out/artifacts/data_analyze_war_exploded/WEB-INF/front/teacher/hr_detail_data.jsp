<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <title>各单位人力资源现状 </title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="Free HTML5 Template by FREEHTML5.CO" />
    <meta name="keywords" content="free html5, free template, free bootstrap, html5, css3, mobile first, responsive" />
    <meta name="author" content="FREEHTML5.CO" />

    <!-- Facebook and Twitter integration -->
  <meta property="og:title" content=""/>
  <meta property="og:image" content=""/>
  <meta property="og:url" content=""/>
  <meta property="og:site_name" content=""/>
  <meta property="og:description" content=""/>
  <meta name="twitter:title" content="" />
  <meta name="twitter:image" content="" />
  <meta name="twitter:url" content="" />
  <meta name="twitter:card" content="" />
    <!-- Place favicon.ico and apple-touch-icon.png in the root directory -->
    <link rel="shortcut icon" href="${pageContext.request.contextPath}/resource/front/lib_one/favicon.ico">

    <!-- Google Webfont -->
    <!--
    <link href='http://fonts.useso.com/css?family=PT+Mono' rel='stylesheet' type='text/css'>
     -->
    <!-- Themify Icons -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/themify-icons.css">
    <!-- Icomoon Icons -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/icomoon-icons.css">
    <!-- Bootstrap -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/bootstrap.css">
    <!-- Owl Carousel -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/owl.carousel.min.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/owl.theme.default.min.css">
    <!-- Magnific Popup -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/magnific-popup.css">
    <!-- Easy Responsive Tabs -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/easy-responsive-tabs.css">
    <!-- Theme Style -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resource/front/lib_one/css/style.css">


    <!-- FOR IE9 below -->
    <!--[if lte IE 9]>
    <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/modernizr-2.6.2.min.js"></script>
    <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/respond.min.js"></script>
    <![endif]-->

  <script src="${pageContext.request.contextPath}/resource/front/chart_lib/Chart.bundle.js"></script>
  <script src="${pageContext.request.contextPath}/resource/front/chart_lib/samples/utils.js"></script>



</head>

<body class="fh5co-outer">
  <header id="fh5co-header" role="banner">
    
      <nav class="navbar navbar-default navbar-fixed-top" role="navigation">
        <div class="container">
          <div class="navbar-header"> 
            <!-- Mobile Toggle Menu Button -->
            <a href="#" class="js-fh5co-nav-toggle fh5co-nav-toggle" data-toggle="collapse" data-target="#fh5co-navbar" aria-expanded="false" aria-controls="navbar"><i></i></a>
            <a class="navbar-brand" href="${pageContext.request.contextPath}/">返回主页</a>
          </div>
          <div id="fh5co-navbar" class="navbar-collapse collapse">
            <ul class="nav navbar-nav">
            </ul>
            <ul class="nav navbar-nav navbar-right">

              <li>


              </li>
              <li>&nbsp;&nbsp;</li>
              <li>&nbsp;&nbsp;</li>
              <li><button class="btn btn-primary">登录</button></li>
            </ul>
          </div>
        </div>
      </nav>

  </header>


  <div id="fh5co-page-title" style="background-image: url(${pageContext.request.contextPath}/resource/front/lib_one/images/index_bg.jpg)">
    <div class="overlay"></div>
    <div class="container">
      <div class="text">
        <h1>各单位人力资源现状</h1>
      </div>

    </div>
  </div>


  <main role="main" id="fh5co-main">

	<div class="copyrights">copy right @ <a href="#" >UP-VAMS工作室</a></div>
    <section class="grid-gallery">
      <div class="container">

        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算机科学与技术系教授类别分布</h5>


            <div id="canvas-holder">
              <canvas id="chart-area" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算机科学与技术系副教授类别分布</h5>


            <div id="canvas-holder1">
              <canvas id="chart-area1" />
            </div>



          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算机科学与技术系讲师类别分布</h5>


            <div id="canvas-holder2">
              <canvas id="chart-area2" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center"></h5>


            <div id="canvas-holder3">
              <canvas id="chart-area3" />
            </div>



          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电气与自动化工程系教授类别分布</h5>


            <div id="canvas-holder4">
              <canvas id="chart-area4" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电气与自动化工程系副教授类别分布</h5>


            <div id="canvas-holder5">
              <canvas id="chart-area5" />
            </div>



          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电气与自动化工程系讲师类别分布</h5>


            <div id="canvas-holder6">
              <canvas id="chart-area6" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center"></h5>


            <div id="canvas-holder7">
              <canvas id="chart-area7" />
            </div>



          </div>
        </div>
        <br/>
        <hr/>
        <br/>

        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电子信息工程系教授类别分布</h5>

            <div id="canvas-holder8">
              <canvas id="chart-area8" />
            </div>

          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电子信息工程系副教授类别分布</h5>

            <div id="canvas-holder9">
              <canvas id="chart-area9" />
            </div>

          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电子信息工程系讲师类别分布</h5>

            <div id="canvas-holder10">
              <canvas id="chart-area10" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center"></h5>


            <div id="canvas-holder11">
              <canvas id="chart-area11" />
            </div>


          </div>
        </div>
        <br/>
        <hr/>
        <br/>



        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算中心教授类别分布</h5>

            <div id="canvas-holder12">
              <canvas id="chart-area12" />
            </div>

          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算中心副教授类别分布</h5>

            <div id="canvas-holder13">
              <canvas id="chart-area13" />
            </div>

          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">计算中心讲师类别分布</h5>

            <div id="canvas-holder14">
              <canvas id="chart-area14" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center"></h5>


            <div id="canvas-holder15">
              <canvas id="chart-area15" />
            </div>


          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电工电子实验中心教授类别分布</h5>

            <div id="canvas-holder16">
              <canvas id="chart-area16" />
            </div>

          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电工电子实验中心副教授类别分布</h5>

            <div id="canvas-holder17">
              <canvas id="chart-area17" />
            </div>

          </div>
        </div>
        <br/>
        <hr/>
        <br/>


        <div class="row gallery-row">

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center">电工电子实验中心讲师类别分布</h5>

            <div id="canvas-holder18">
              <canvas id="chart-area18" />
            </div>


          </div>

          <div class="col-md-6 col-sm-6">

            <h5 style="text-align: center"></h5>


            <div id="canvas-holder19">
              <canvas id="chart-area19" />
            </div>


          </div>
        </div>
        <br/>
        <hr/>
        <br/>


      </div>
    </section>

  </main>


  <footer role="contentinfo" id="fh5co-footer">
    <div class="container">
      <div class="row">
        <div class="col-md-3 col-sm-6">
          <div class="footer-box border-bottom">
            <h3 class="footer-heading"> 关于我们</h3>
            <p>
               我们是信息工程学院教学数据信息开发的团队。
            </p>
          </div>
          
            <h3 class="footer-heading">订阅</h3>
            <form action="#" class="form-subscribe">
              <div class="field">
                <input type="email" class="form-control" placeholder="hello@mail.com">
                <button class="btn btn-primary">订阅</button>
              </div>
            </form>
            <div class="fh5co-spacer fh5co-spacer-sm"></div>
          
        </div>
        <div class="col-md-3 col-sm-6">
          
            <h3 class="footer-heading">正在开发的项目</h3>
            <ul class="footer-list">
              <li><a href="#">项目一</a></li>
              <li><a href="#">项目二</a></li>
              <li><a href="#">项目三</a></li>
            </ul>
          
        </div>


        <div class="visible-sm clearfix"></div>


        <div class="col-md-3 col-sm-6">
          
            <h3 class="footer-heading">初步成果</h3>
            <ul class="footer-list">
              <li><a href="#"><abbr title="Hypertext Markup Language 5">成果一</abbr></a></li>
              <li><a href="#"><abbr title="Cascading Stylesheets 3">成果二</abbr></a></li>
              <li><a href="#">成果三</a></li>
              <li><a href="#">成果四</a></li>
            </ul>
          
        </div>


        <div class="col-md-3 col-sm-6 clearfix">

          <div class="row">
            <div class="col-md-6 col-sm-6">
              <div class="footer-box">
                <h3 class="footer-heading">联系我们</h3>
                <ul class="footer-list">
                  <li><a href="#">我们的主页</a></li>
                  <li><a href="#">给我们留言</a></li>
                  <li><a href="#">关于我们</a></li>
                  
                </ul>
              </div>
            </div>
            <div class="col-md-6 col-sm-6">
              <div class="footer-box">
                <h3 class="footer-heading">技术支持</h3>
                <ul class="footer-list">
                  <li><a href="#">问题</a></li>
                  <li><a href="#">领域知识</a></li>
                </ul>
              </div>
            </div>
          </div>
          
        </div>
        
      </div>
    </div>
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <div class="copyright">
            <p class="text-center">
              <a href="#" target="_blank">UP-VAMS</a><br> &copy; 2017 Display. All Rights Reserved.</p>
          </div>
        </div>
      </div>
    </div>
  </footer>

  <script type="text/javascript">

      //      ==========================================================================================

      <!--计算机科学与技术系教授类别分布-->
      var barChartData = {
          labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
          datasets: [{
              label: '45岁及以上',
              backgroundColor: window.chartColors.red,
              data: [
                  0,
                  9,
                  0,
                  1,
                  4,
                  1,
                  4
              ]
          }, {
              label: '44岁及以下',
              backgroundColor: window.chartColors.blue,
              data: [
                  0,
                  3,
                  0,
                  0,
                  2,
                  2,
                  0
              ]
          }
          ]

      };

      //      ==========================================================================================

      //    计算机科学与技术系副教授类别分布
    var barChartData1 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                2,
                7,
                0,
                0,
                1,
                3,
                4
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                8,
                0,
                0,
                3,
                6,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================


      //    计算机科学与技术系讲师类别分布
    var barChartData2 = {
        labels: [ "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                2,
                6,
                1
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                9,
                5,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================

      //      ==========================================================================================


      <!--电气与自动化系教授类别分布-->
    var barChartData3 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                0,
                11,
                0,
                0,
                4,
                5,
                2
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                1,
                0,
                0,
                1,
                0,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================


      //    电气与自动化副教授类别分布
    var barChartData4 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                0,
                8,
                0,
                0,
                4,
                3,
                1
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                9,
                0,
                0,
                6,
                3,
                0
            ]
        }
        ]

    };


      //      ==========================================================================================


      //    电气与自动化讲师类别分布
    var barChartData5 = {
        labels: [ "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                0,
                1,
                0
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                6,
                3,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================

      //      ==========================================================================================


      <!--电子信息工程系教授类别分布-->
    var barChartData6 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                1,
                9,
                0,
                0,
                2,
                3,
                5
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                6,
                0,
                0,
                4,
                0,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================






      // 电子信息工程系副教授类别分布
    var barChartData7 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                5,
                3,
                0,
                1,
                0,
                3,
                5
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                5,
                0,
                0,
                9,
                0,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================



      // 电子信息工程系讲师类别分布
    var barChartData8 = {
        labels: [ "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                2,
                6,
                1
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                9,
                5,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================


//
    <!--计算中心教授类别分布-->
    var barChartData9 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                3,
                2,
                0,
                0,
                0,
                1,
                3
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                1,
                0,
                0,
                1,
                0,
                0
            ]
        }
        ]

    };


      //      ==========================================================================================


      // 计算中心副教授类别分布
    var barChartData10 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                8,
                6,
                0,
                0,
                0,
                7,
                6
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                7,
                0,
                0,
                1,
                4,
                2
            ]
        }
        ]

    };

      //      ==========================================================================================


      // 计算中心讲师类别分布
    var barChartData11 = {
        labels: [ "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                0,
                1,
                0
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                2,
                8,
                1
            ]
        }
        ]

    };


      //      ==========================================================================================


      <!--电工电子实验中心教授类别分布-->
    var barChartData12 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                1,
                1,
                0,
                0,
                1,
                1,
                0
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                0,
                0,
                0,
                0,
                0,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================


      // 电工电子实验中心副教授类别分布
    var barChartData13 = {
        labels: ["基础教学型", "教学科研并重", "科研型", "社会服务型", "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                2,
                0,
                0,
                0,
                0,
                0,
                2
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                0,
                4,
                0,
                0,
                2,
                2,
                0
            ]
        }
        ]

    };

      //      ==========================================================================================


      // 电工电子实验中心讲师类别分布
    var barChartData14 = {
        labels: [ "博士", "硕士", "学士"],
        datasets: [{
            label: '45岁及以上',
            backgroundColor: window.chartColors.red,
            data: [
                0,
                0,
                1
            ]
        }, {
            label: '44岁及以下',
            backgroundColor: window.chartColors.blue,
            data: [
                6,
                0,
                0
            ]
        }
        ]

    };

    window.onload = function() {


          var ctx = document.getElementById("chart-area").getContext("2d");
          var ctx1 = document.getElementById("chart-area1").getContext("2d");
          var ctx2 = document.getElementById("chart-area2").getContext("2d");

          var ctx4 = document.getElementById("chart-area4").getContext("2d");
          var ctx5 = document.getElementById("chart-area5").getContext("2d");
          var ctx6 = document.getElementById("chart-area6").getContext("2d");

          var ctx8 = document.getElementById("chart-area8").getContext("2d");
          var ctx9 = document.getElementById("chart-area9").getContext("2d");
          var ctx10 = document.getElementById("chart-area10").getContext("2d");

          var ctx12 = document.getElementById("chart-area12").getContext("2d");
          var ctx13 = document.getElementById("chart-area13").getContext("2d");
          var ctx14 = document.getElementById("chart-area14").getContext("2d");

          var ctx16 = document.getElementById("chart-area16").getContext("2d");
          var ctx17 = document.getElementById("chart-area17").getContext("2d");
          var ctx18 = document.getElementById("chart-area18").getContext("2d");


        window.myBar = new Chart(ctx, {
              type: 'bar',
              data: barChartData,
              options: {
                  title:{
                      display:true,
                  },
                  tooltips: {
                      mode: 'index',
                      intersect: false
                  },
                  responsive: true,
                  scales: {
                      xAxes: [{
                          stacked: true,
                      }],
                      yAxes: [{
                          stacked: true
                      }]
                  }
              }
          });

          window.myBar1 = new Chart(ctx1, {
              type: 'bar',
              data: barChartData1,
              options: {
                  title:{
                      display:true,
                  },
                  tooltips: {
                      mode: 'index',
                      intersect: false
                  },
                  responsive: true,
                  scales: {
                      xAxes: [{
                          stacked: true,
                      }],
                      yAxes: [{
                          stacked: true
                      }]
                  }
              }
          });

        window.myBar2 = new Chart(ctx2, {
            type: 'bar',
            data: barChartData2,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });


        window.myBar3 = new Chart(ctx4, {
            type: 'bar',
            data: barChartData3,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar4 = new Chart(ctx5, {
            type: 'bar',
            data: barChartData4,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar5 = new Chart(ctx6, {
            type: 'bar',
            data: barChartData5,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });


        window.myBar6 = new Chart(ctx8, {
            type: 'bar',
            data: barChartData6,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar7 = new Chart(ctx9, {
            type: 'bar',
            data: barChartData7,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar8 = new Chart(ctx10, {
            type: 'bar',
            data: barChartData8,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

//      ==========================================================================================

        window.myBar9 = new Chart(ctx12, {
            type: 'bar',
            data: barChartData9,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar10 = new Chart(ctx13, {
            type: 'bar',
            data: barChartData10,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar11 = new Chart(ctx14, {
            type: 'bar',
            data: barChartData11,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

//

        window.myBar12 = new Chart(ctx16, {
            type: 'bar',
            data: barChartData12,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar13 = new Chart(ctx17, {
            type: 'bar',
            data: barChartData13,
            options: {
                title:{
                    display:true,
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

        window.myBar14 = new Chart(ctx18, {
            type: 'bar',
            data: barChartData14,
            options: {
                title:{
                    display:true,
//                      text:"Chart.js Bar Chart - Stacked"
                },
                tooltips: {
                    mode: 'index',
                    intersect: false
                },
                responsive: true,
                scales: {
                    xAxes: [{
                        stacked: true,
                    }],
                    yAxes: [{
                        stacked: true
                    }]
                }
            }
        });

      };


  </script>


  <!-- Go To Top -->
    <a href="#" class="fh5co-gotop"><i class="ti-shift-left"></i></a>



  <!-- jQuery -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/jquery-1.10.2.min.js"></script>
  <!-- jQuery Easing -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/jquery.easing.1.3.js"></script>
  <!-- Bootstrap -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/bootstrap.js"></script>
  <!-- Owl carousel -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/owl.carousel.min.js"></script>
  <!-- Magnific Popup -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/jquery.magnific-popup.min.js"></script>
  <!-- Easy Responsive Tabs -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/easyResponsiveTabs.js"></script>
  <!-- FastClick for Mobile/Tablets -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/fastclick.js"></script>

  <!-- Main JS -->
  <script src="${pageContext.request.contextPath}/resource/front/lib_one/js/main.js"></script>

</body>
</html>
